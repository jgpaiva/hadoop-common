...
public class BlockPlacementPolicyRowGroup extends BlockPlacementPolicy {
  protected boolean considerLoad; 
  protected NetworkTopology clusterMap;
  private FSClusterStats stats;
  private long staleInterval;   // interval used to identify stale DataNodes
  
  HashMap<String,DatanodeDescriptor[]> replicationMap;//Create at initialization
  HashMap<String, Integer> countMap;////Create at initialization
  int totalColumnCount;//Set the value at initialization from the configuration
  String filePath; //Set at chooseTarget method.

  BlockPlacementPolicyRowGroup(Configuration conf,  FSClusterStats stats,
                           NetworkTopology clusterMap) {
    initialize(conf, stats, clusterMap);
  }

  BlockPlacementPolicyRowGroup() {
  }
  ...
  public void initialize(Configuration conf,  FSClusterStats stats,
    ...
    this.totalColumnCount=conf.getInt(
    		DFSConfigKeys.DFS_BLOCK_ROWGROUP_TOTAL_COLUMN_COUNT_KEY,
    		DFSConfigKeys.DFS_BLOCK_ROWGROUP_TOTAL_COLUMN_COUNT_DEFAULT);
  }
  ...
  /**
   * This extracts the key from the path below is an example
   * filePath= /hadoop/attempt_201406251439_0001_r_000000_0/col10_lesbosZblockZ2-r-00002.gz";
   * return = lesbosZblockZ2
   */
  private String GetMapKeyByFilePath()
  {
		String fileName= filePath.substring(filePath.lastIndexOf('/') + 1);		
		String lastPortion=fileName.substring(fileName.lastIndexOf('_') + 1);		
		return lastPortion.substring(0,lastPortion.indexOf('-'));
  }
  ...
  DatanodeDescriptor[] chooseTarget(int numOfReplicas,
	...
	String mapKey=null;//This can be obtained from fileName.
	
	if(filePath!=null && filePath.contains("ZblockZ") && filePath.contains(".gz"))
	{			
		mapKey=GetMapKeyByFilePath();
		if(replicationMap!=null && replicationMap.containsKey(mapKey))
		{
			//if replicationMap has the key countMap also has the key			
			int currentCount=countMap.get(mapKey);
			DatanodeDescriptor[] tempResult=replicationMap.get(mapKey);
			if((currentCount+1)==totalColumnCount)
			{		
				 FSNamesystem.LOG.info("removed "+mapKey+ " count "+currentCount);
				replicationMap.remove(mapKey);
				countMap.remove(mapKey);
				if(replicationMap.size()==0){
					replicationMap=null;
					countMap=null;
				}
			}
			...
			{
				 FSNamesystem.LOG.info("incremented "+mapKey+ " count "+currentCount);
				countMap.put(mapKey, (currentCount+1));
			}
			return tempResult;
		}
	}
    ...
    DatanodeDescriptor[] resultDescriptors=getPipeline((writer==null)?localNode:writer,
    ...
    if(filePath!=null && filePath.contains("ZblockZ") && filePath.contains(".gz"))
    {
    	...
    	if(replicationMap==null){
    		replicationMap=new HashMap<String, DatanodeDescriptor[]>();
    		countMap=new HashMap<String, Integer>();
    	}
    	replicationMap.put(mapKey, resultDescriptors);
