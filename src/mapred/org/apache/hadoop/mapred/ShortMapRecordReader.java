...
public class ShortMapRecordReader implements RecordReader<LongWritable, Text> {
			...
			if (isCompressedInput()) {
				...
				if (!(codec instanceof SplittableCompressionCodec)) {
					...
					for (FSDataInputStream fileInAttr : array2inputStreams) {
						...
						if (offsetList.get(i) != 0) {// Offset to be correct,
	...
	/**
	 * @param rowGroupName
	 *            Return the list of relevant paths and the first file path
	 */
	private ArrayList<Path> getRelavantAttrPaths(Path file, String rowGroupName) {
		...
		LOG.info("Relavant config is " + relevantAttrStr);
	...
	public boolean next(LongWritable key, Text value) throws IOException {
		while (getFilePosition() <= end) {
			if (stop()) {// we should process only if there is a record to
							// process
				return false;
			}
			key.set(pos);
			...
			System.out.println("combimed file count is " + inN.size());
